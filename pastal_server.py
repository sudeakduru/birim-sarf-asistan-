...
# Mathematical marker layout calculation engine
def calculate_marker_cost(data, apply_learning=True):
    ...
    if apply_learning:
        ...
        cursor.execute("""
            ...
            WHERE m.Urun_Grubu = ? 
              AND c.Gerceklesen_Birim_Metraj_M IS NOT NULL 
    ...
    for size_name, qty in asorti.items():
        ...
        if "alt" in urun_grubu_lower:
            bel = get_val(m, ["bel", "waist", "w14", "w13", "w222", "bel_kavisli_genisligi_ustten", "bel_genisligi_kavisli_alttan", "bel_genisligi", "bel_kavisli_genisligi"], 60.0)
            basen = get_val(m, ["basen", "hip", "hips", "w59", "basen_genisligi"], 80.0)
            on_ag = get_val(m, ["on ag", "on_ag", "front rise", "front_rise", "ag", "l19", "on_ag_uzunlugu_kemer_dahil", "on_ag_uzunlugu", "on_ag_kemer_dahil"], 20.0)
            arka_ag = get_val(m, ["arka ag", "arka_ag", "back rise", "back_rise", "l21", "arka_ag_uzunlugu_kemer_dahil", "arka_ag_uzunlugu", "arka_ag_kemer_dahil"], on_ag * 1.15)
            ...
            paca_eni = get_val(m, ["paca eni", "paca_eni", "hem width", "hem_width", "paca", "w252", "w178", "paca_genisligi_ic_boy_16_40_cm_arasi", "paca_genisligi", "paca_genisligi_ic_boy"], 25.0)
            ...
            # Basen düşüklüğü yoksa ön ağ yüksekliğinden tahmin et
            if basen_drop <= 0:
            ...
            # ÖN PANO: Belden basene yamuk + basenden paca'ya yamuk
            # Ön ağ çıkıntısı pano genişliğine katkı yapar
            on_bel = bel / 4.0
            on_basen = basen / 4.0 + on_ag * 0.25  # ön ağ genişlik katkısı
            on_paca = paca_eni
            
            on_ust_alan = (on_bel + on_basen) / 2.0 * basen_drop  # belden basene
            on_alt_alan = (on_basen + on_paca) / 2.0 * bacak_boy   # basenden paca'ya
            
            # ARKA PANO: Arka ağ daha büyük çıkıntı yapar
            arka_bel = bel / 4.0 + 2.0  # arka bel biraz daha geniş (pens farkı)
            arka_basen = basen / 4.0 + arka_ag * 0.35  # arka ağ daha büyük çıkıntı
            arka_paca = paca_eni
            
            arka_ust_alan = (arka_bel + arka_basen) / 2.0 * basen_drop
            arka_alt_alan = (arka_basen + arka_paca) / 2.0 * bacak_boy
            
            # 4 pano toplam: 2 ön (sağ+sol) + 2 arka (sağ+sol)
            ...
            max_piece_width_cm = max(max_piece_width_cm, arka_basen, on_basen, paca_eni)
            ...
            # Kol genişliği: kolevi_duz varsa onu kullan, yoksa boy/3 varsay
            sleeve_width = get_val(m, ["kolevi_duz", "kolevi", "kol_agzi_genisligi_kisa_kol_gergin", "sleeve_width"], boy / 3.0)
            ...
            etek_eni = get_val(m, ["etek eni", "etek_eni", "hem width", "hem_width", "etek"], 60.0)
            
            # Check if bodice height is specified (to split body into bodice and skirt)
            ...
            if sleeve_width == 20.0 and boy < 75.0: # child/baby size clamp
                sleeve_width = 14.0
            area_kol = 2.0 * kol_boyu * sleeve_width * 1.15 / 10000.0 if kol_boyu > 0 else 0.0
...
class PastalHTTPHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path == "/api/modeller":
            ...
            conn.close()
            self.wfile.write(json.dumps(kayitlar).encode('utf-8'))
            return
            ...
            try:
                ...
                if mime_type == "application/pdf":
                    try:
                        ...
                        # Always prefer multimodal (visual) API for measurement tables
                        # because pypdf text extraction mangles table column alignment
                        print(f"[PDF PARSER] Skipping local text parse ({len(combined_text)} chars). Using multimodal (visual) API for accurate table reading...")
                        response_text = None
